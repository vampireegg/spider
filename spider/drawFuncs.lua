...
local function drawScoreBoard(sceneGroup, scoreboard, scoreboardProp, totalWidth, totalHeight)
	...
	scoreboard.scoreText[8] = display.newText( "Time : " .. scoreboardProp.SpentTime .. "s", posiX, posiY,  "comic.ttf", 12 )
		...
		for j = 1, 2 do
			...
			for k = 1, switchSystemProp.window.Num[i][j] do
				...
				if(switchSystemProp.window.CurState[i][j] == 0) then
					switchSystem.window[i][j][k]:setFillColor( switchSystemProp.Color[i][1], switchSystemProp.Color[i][2], switchSystemProp.Color[i][3], 0.15 )
				else
